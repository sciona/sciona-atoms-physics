...
```bash
python scripts/reseed_physics_supabase.py --only pdg_remote_wave5_20260505
```
...
So far, 25 remote derivations have been promoted through symbolic expression
rows, PDG relationship rows, and CDG rows across five applied waves.
...
The latest wave, `pdg_remote_wave5_20260505`, promoted:

- `000014`: variance relation
- `000016`: identity `sin(2 x) = 2 sin(x) cos(x)` using Euler's equation
- `000017`: Euler equation to `e^(i pi) + 1 = 0`
- `375160`: coefficient of isothermal compressibility using the ideal gas EOS
- `681943`: total electrical resistance for two resistors in parallel

The wave 5 dry run and apply both completed with zero graph diagnostics and
zero skipped PDG edges. The apply used authenticated GitHub requests via
`gh-auth-token`; the last observed remaining quota was 4,970 requests.
