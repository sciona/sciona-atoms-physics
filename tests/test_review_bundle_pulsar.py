...
BUNDLE_PATH = REPO_ROOT / "docs/review-bundles/pulsar_pipeline_review_bundle.json"


def test_pulsar_review_bundle_maps_to_registered_atoms() -> None:
    bundle = json.loads(BUNDLE_PATH.read_text())
    registry = importlib.import_module("sciona.ghost.registry").REGISTRY

    assert bundle["family"] == "sciona.atoms.physics.pulsar.pipeline"
