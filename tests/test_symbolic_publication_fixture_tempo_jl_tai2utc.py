...
FIXTURE_PATH = (
    ...
    / "tempo_jl_tai2utc.publication_manifest.json"
)
EXPECTED_NAMES = {
    "cal2jd",
    "calhms2jd",
...
def test_tempo_jl_tai2utc_fixture_surfaces_symbolic_metadata() -> None:
    fixture = _fixture()
    expressions = {
    ...
    assert fixture["modules"] == ["sciona.atoms.physics.tempo_jl.tai2utc.atoms"]
    ...
    inverse = expressions["tai2utc"]
    assert (
        inverse["raw_formula"]
        == "Eq(utc_total, tai1 + tai2 - delta_at/day_seconds)"
    )
    assert inverse["constants"] == {"day_seconds": 86400.0}
    assert inverse["mechanism_tags"] == [
        "leap_second",
        "tai_utc_conversion",
        "time_scale_conversion",
    ]
    assert variables[("tai2utc", "delta_at")]["variable_role"] == "parameter"
    assert variables[("tai2utc", "utc_total")]["variable_role"] == "output"


def test_tempo_jl_tai2utc_fixture_matches_live_symbolic_manifest() -> None:
    fixture = _fixture()
...
def test_tempo_jl_tai2utc_fixture_loads_without_database_io() -> None:
    fixture = _fixture()
    artifact_ids = {
        row["artifact_key"]: {
